	...
	if [ "$OS" == "Debian" ]; then	
		if [ "${recordStereo}" != "1" ]; then
			...
			if [ "$PositionAgent" == "left" ]; then
				...
				echo "$cmdDebian $recordingremotedir/$queueName/$in_file $recordingremotedir/$queueName/$out_file /GrabacionesWAV/$queueName/$callId.wav && " >> $recordingdir/$callId.prs
			else
				...
				echo "$cmdDebian $recordingremotedir/$queueName/$out_file $recordingremotedir/$queueName/$in_file /GrabacionesWAV/$queueName/$callId.wav && " >> $recordingdir/$callId.prs
			fi
		fi
	else
