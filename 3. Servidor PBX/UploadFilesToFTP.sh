...
main(){	
	...
	do
		...
		if [[ -f $inFile ]]; then
			...
			#remotedir=$remotedirPath"/"$(date -r $inFile '+%Y' | bc)"/"$(date -r $inFile '+%m' | bc)"/"$(date -r $inFile '+%d' | bc)"/"$(date -r $inFile '+%H' | bc)
			remotedir=$remotedirPath"/"$(date -d "$(date -r $inFile) - 5 hours" '+%Y' | bc)"/"$(date -d "$(date -r $inFile) - 5 hours" '+%m' | bc)"/"$(date -d "$(date -r $inFile) - 5 hours" '+%d' | bc)"/"$(date -d "$(date -r $inFile) - 5 hours" '+%H' | bc)
